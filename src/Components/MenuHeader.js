...
export default function MenuHeader() {
  ...
  if (!localStorage.getItem("login_user"))
    ...
    controlContent = (
      <div className="items-center flex-shrink-0 hidden lg:flex">
        <Button
          size="large"
          type="primary"
          className="self-center px-8 py-3 m-3 rounded"
          onClick={() => {
            navigate("/login")
          }}
        >
    ...
    controlContent = (
      ...
      }} className="flex duration-500 align-middle text-violet hover:text-red">
  ...
  return (
    <div>
      <div className="fixed w-full z-30 p-4 dark:bg-coolGray-800 dark:text-coolGray-100 bg-black bg-opacity-70">
        <div className="container flex justify-between h-10 mx-auto">
          <a
            rel="noopener noreferrer"
            href="CC"
            aria-label="Back to homepage"
            className="flex items-center p-2 text-violet"
          >
            <svg
          ...
          <ul className="items-stretch hidden space-x-3 lg:flex ">
            <li className="flex">
              <NavLink
                to={"/home"}
                className="text-violet flex items-center px-4 -mb-1 dark:border-transparent dark:text-violet-400 dark:border-violet-400"
              >
                Home
              </NavLink>
            </li>
            <li className="flex">
              <NavLink
                to={"/news"}
                className="text-violet flex items-center px-4 -mb-1  dark:border-transparent"
              >
                News
              </NavLink>
            </li>
